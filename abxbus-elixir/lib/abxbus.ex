defmodule Abxbus do
  ...
  **Queue-jump**: If called from within a handler, the awaited event will
  ...
  """
  def current_bus! do
    case Process.get(:abxbus_current_bus) do
  ...
  def current_event_id, do: Process.get(:abxbus_current_event_id)

  @doc "Get the current handler ID from handler context, or nil."
  def current_handler_id, do: Process.get(:abxbus_current_handler_id)

  @doc "Check whether we're inside an event handler."
  def in_handler_context?, do: Process.get(:abxbus_current_event_id) != nil
  ...
  defp maybe_set_parent(%{event_parent_id: nil} = event) do
    ...
    if event.event_path != [] do
      ...
      case Process.get(:abxbus_current_event_id) do
        nil -> event
