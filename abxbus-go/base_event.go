...
func (e *BaseEvent) MarshalJSON() ([]byte, error) {
	...
	if len(e.EventResults) > 0 {
		...
		for _, result := range e.sortedEventResults() {
			results[result.HandlerID] = result
		}
		record["event_results"] = results
	}
