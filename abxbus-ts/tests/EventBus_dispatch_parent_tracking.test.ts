...
test('basic parent tracking: child events get event_parent_id', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    const child = event.emit(ChildEvent({ data: `child_of_${event.message ?? 'root'}` }))
    if (!child) {
...
test('multi-level parent tracking preserves lineage', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    event.emit(ChildEvent({ data: 'child_data' }))
    return 'parent'
  })

  bus.on(ChildEvent, (event) => {
    event.emit(GrandchildEvent({ value: 42 }))
    return 'child'
  })
...
test('multiple children from same parent keep same event_parent_id', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    for (let i = 0; i < 3; i += 1) {
      const child = event.emit(ChildEvent({ data: `child_${i}` }))
      if (!child) {
        throw new Error('expected scoped bus on parent handler event')
      }
      child_events.push(child)
...
test('bus.emit outside handler does not guess a parent when multiple handlers are active', async () => {
  ...
  release_a()
  release_b()
  await Promise.all([parent_a.now(), parent_b.now(), unrelated_child.now()])
...
test('parallel parent handlers preserve parent tracking', async () => {
  ...
  bus.on(ParentEvent, async (event) => {
    await delay(10)
    const child = event.emit(ChildEvent({ data: 'from_h1' }))
    if (!child) {
      throw new Error('expected scoped bus on parent handler event')
    }
  ...
  bus.on(ParentEvent, async (event) => {
    await delay(20)
    const child = event.emit(ChildEvent({ data: 'from_h2' }))
    if (!child) {
      throw new Error('expected scoped bus on parent handler event')
    }
...
test('parent tracking works with sync handlers and handler errors', async () => {
  ...
  const sync_handler = (event: BaseEvent): string => {
    const child = event.emit(ChildEvent({ data: 'from_sync' }))
    if (!child) {
      throw new Error('expected scoped bus on parent handler event')
    }
    child_events.push(child)
  ...
  const failing_handler = (event: BaseEvent): never => {
    const child = event.emit(ChildEvent({ data: 'before_error' }))
    if (!child) {
      throw new Error('expected scoped bus on parent handler event')
    }
    child_events.push(child)
  ...
  const success_handler = (event: BaseEvent): string => {
    const child = event.emit(ChildEvent({ data: 'after_error' }))
    if (!child) {
      throw new Error('expected scoped bus on parent handler event')
    }
    child_events.push(child)
...
test('event_children tracks direct and nested descendants', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    event.emit(ChildEvent({ data: 'level1' }))
    return 'parent'
  })
...
test('event_children tracks multiple children from a single handler', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    event.emit(ChildEvent({ data: 'child_0' }))
    event.emit(ChildEvent({ data: 'child_1' }))
    event.emit(ChildEvent({ data: 'child_2' }))
    return 'parent_done'
  })
...
test('multiple parent handlers contribute to one event_children list', async () => {
  ...
  bus.on(ParentEvent, (event) => {
    event.emit(ChildEvent({ data: 'from_handler_1' }))
    return 'h1'
  })
  bus.on(ParentEvent, (event) => {
    event.emit(ChildEvent({ data: 'from_handler_2a' }))
    event.emit(ChildEvent({ data: 'from_handler_2b' }))
    return 'h2'
  })
  bus.on(ChildEvent, (event) => `handled_${event.data ?? 'unknown'}`)
...
test('parent completion waits for awaited children', async () => {
  ...
  try {
    ...
    for (const child of parent.event_children) {
      assert.equal(child.event_blocks_parent_completion, true)
...
test('bus.emit inside a handler dispatches a root event by default', async () => {
  ...
  bus.on(ParentEvent, () => {
    child_ref = bus.emit(ChildEvent({ data: 'detached' }))
    return 'parent'
  })
  bus.on(ChildEvent, async () => {
    child_started_resolve?.()
  ...
  const parent = bus.emit(ParentEvent({ message: 'detached' }))
  try {
    await child_started
    assert.ok(child_ref)
...
test('outside await of bus.emit child.now() keeps it independent of the active handler', async () => {
  ...
  try {
    ...
    const child_done = child_ref.now()
    assert.equal(child_ref.event_blocks_parent_completion, false)
...
test('outside wait of bus.emit child keeps it independent of the active handler', async () => {
  const bus = new EventBus('RootChildExternalWaitBus')
  let parent_holding_resolve: (() => void) | null = null
  const parent_holding = new Promise<void>((resolve) => {
    parent_holding_resolve = resolve
  })
  ...
  bus.on(ParentEvent, async () => {
    child_ref = bus.emit(ChildEvent({ data: 'external_wait' }))
    parent_holding_resolve?.()
    await parent_released
    return 'parent'
  })
  ...
  const parent = bus.emit(ParentEvent({ message: 'external_wait' }))
  try {
    await parent_holding
    assert.ok(child_ref)
    assert.equal(child_ref.event_parent_id, null)
    ...
    const child_completed = child_ref.wait()
    assert.equal(child_ref.event_blocks_parent_completion, false)
...
test('awaited bus.emit child remains independent and does not block parent completion', async () => {
  ...
  await bus.emit(ParentEvent({ message: 'awaited' })).now()
  assert.ok(child_ref)
  assert.equal(child_ref.event_parent_id, null)
  assert.equal(child_ref.event_blocks_parent_completion, false)
})
