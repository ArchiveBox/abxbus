...
```rust
use abxbus_rust::{event, BaseEventHandle};
...
let event = BaseEventHandle::<FooCreateEvent>::new(FooCreateEvent {
    id: None,
    name: "Ada".to_string(),
    age: 37,
});
...
In Rust the same split is exposed as `event.done().await` for immediate queue-jump execution and `event.event_completed().await` for normal queue-order waiting.
...
let pending = bus.emit(BaseEventHandle::<MyEvent>::new(MyPayload::default()));
...
let event = BaseEventHandle::<AllowedEvent>::new(AllowedEvent {
