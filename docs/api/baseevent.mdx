...
event, err := abxbus.NewTypedEventWithResult[FooPayload, FooResult]("FooCreateEvent", FooPayload{
	Name: "Ada",
	Age:  37,
})
```
...
```rust
use abxbus_rust::typed::{EventSpec, TypedEvent};
...
impl EventSpec for FooCreateEvent {
    type Payload = FooPayload;
    type Result = String;

    const EVENT_TYPE: &'static str = "FooCreateEvent";
}

let event = TypedEvent::<FooCreateEvent>::new(FooPayload {
    id: None,
    name: "Ada".to_string(),
    age: 37,
});
...
```rust
use abxbus_rust::base_event::{BaseEvent, EventResultsOptions};
use futures::executor::block_on;

let pending = bus.emit_base(BaseEvent::new("MyEvent", Default::default()));
...
```rust
use abxbus_rust::base_event::BaseEvent;
use serde_json::{json, Map};

let mut payload = Map::new();
payload.insert("payload".to_string(), json!("ok"));
let event = BaseEvent::new("AllowedEvent", payload);
event.inner.lock().event_timeout = Some(30.0);

// Rust validates unknown event_* / model_* keys in BaseEvent::new(...)
