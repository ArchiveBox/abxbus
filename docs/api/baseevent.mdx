...
- `event_handler_slow_timeout`: per-handler slow warning threshold (`0` disables)
- `event_slow_timeout`: per-event slow warning threshold (`0` disables)
...
let value = block_on(pending.event_result(EventResultOptions::default()))?;
```
...
let value = block_on(completed.event_result(EventResultOptions::default()))?;
```
...
```rust
let items = block_on(event.event_results_list(EventResultOptions {
    raise_if_any: false,
    raise_if_none: false,
