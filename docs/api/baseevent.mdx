...
You can create a fresh pending copy for re-emit. By default this regenerates ids/routing lineage, resets status and timestamps, clears handler results, and clears runtime attachment state. Pass a field group as `false` to preserve it on the returned copy.
