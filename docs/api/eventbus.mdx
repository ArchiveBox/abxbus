...
```go
bus.OnEventName("UserEvent", "handler", handler, nil)
bus.OnEventName("*", "wildcard_handler", wildcardHandler, nil)
timeout := 5.0
bus.OnEventName("UserEvent", "custom", handler, &abxbus.EventHandler{
...
```go
handler := bus.OnEventName("UserEvent", "handler", onUserEvent, nil)
bus.Off("UserEvent", handler)
...
```go
event := bus.EmitEventName("MyEvent", map[string]any{"data": "x"})
...
```go
event, err := bus.FindEventName("ResponseEvent", nil, nil)
future, err := bus.FindEventName("ResponseEvent", nil, &abxbus.FindOptions{
	Past: false,
	Future: 5.0,
})
child, err := bus.FindEventName("ChildEvent", nil, &abxbus.FindOptions{
	ChildOf: parentEvent,
	Future: 5.0,
})
```
...
recentWithPredicate, err := bus.FilterEventName(
	"ResponseEvent",
...
// 3) Re-link runtime callables by handler id
bus.OnEventName("UserCreatedEvent", "onUserCreated", onUserCreated, bus.Handlers["018f..."])
bus.OnEventName("UserDeletedEvent", "onUserDeleted", onUserDeleted, bus.Handlers["018g..."])
bus.OnEventName("UserUpdatedEvent", "onUserUpdated", onUserUpdated, bus.Handlers["018h..."])
