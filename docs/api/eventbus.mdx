...
let entry = bus.on::<UserEvent, _, _>("handler", |event| async move {
    Ok(json!(event.event_payload().user_id))
});
bus.on_with_options::<UserEvent, _, _>(
    "custom",
    EventHandlerOptions {
        handler_timeout: Some(5.0),
        ..EventHandlerOptions::default()
    },
    |_event| async move { Ok(serde_json::Value::Null) },
);
```
...
let handler = bus.on::<UserEvent, _, _>("handler", |_event| async move {
...
let event = bus.emit(MyEvent { data: "x".to_string() });
