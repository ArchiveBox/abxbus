...
```rust
// EventBus::process_queued_event(...)
event.mark_started();
let handlers = self.handlers_for_event(&event);
let pending_entries = self.create_pending_results(&event, &handlers);

// EventBus::run_handler(...)
result.mark_started();
let handler_result = self
    .run_handler_with_timeout(event.clone(), handler.clone(), result.clone())
    .await;

// BaseEvent::now(...)
EventBus::queue_jump_if_waited(event.clone());
event.event_completed_with_timeout(options.timeout).await;
EventBus::wait_until_event_idle(event.clone(), options.timeout).await;
```
...
All runtimes use the same layered model, expressed with runtime-native wrappers.
...
```go
// EventBus.processEvent(...)
err := runWithTimeout(ctx, resolvedEventTimeout, timeoutError, func(ctx context.Context) error {
	return event.runHandlers(ctx, bus, handlers, pendingEntries, signalFirstHandlerStarted)
})

// BaseEvent.runHandlers(...)
for i, handler := range handlers {
	if err := runSingleHandler(ctx, bus, event, handler, results[i], signalFirstHandlerStarted); err != nil {
		return err
	}
}
...
// runSingleHandler(...)
result.markStarted()
err := runWithTimeout(ctx, resolvedHandlerTimeout, handlerTimeoutError, func(ctx context.Context) error {
	return bus.locks.runWithHandlerDispatchContext(result, func() error {
		returnValue, err = handler.Handle(ctx, event)
		return err
	})
})
```
...
await bus.destroy(timeout=1.0)       # wait up to 1sec, then clear by default
await bus.destroy(clear=False)       # preserve handlers/history if you need to resume
```
...
```go
resumeTick := abxbus.NewBaseEvent("ResumeTickEvent", nil)
...
// 5) Teardown when done
bus.DestroyWithOptions(&abxbus.EventBusDestroyOptions{Clear: true})
```
...
let resume_tick = BaseEvent::new("ResumeTickEvent", Default::default());
...
bus.on_raw("UserCreatedEvent", "on_user_created", |_event| async move { Ok(json!("created")) });
bus.on_raw("UserDeletedEvent", "on_user_deleted", |_event| async move { Ok(json!("deleted")) });
bus.on_raw("UserUpdatedEvent", "on_user_updated", |_event| async move { Ok(json!("updated")) });

// 4) Resume processing
bus.emit_base(resume_tick);
...
- With `clear=false`, destroy stops current runtime work and resolves waiters but preserves handlers/history so the same bus can be used again.
