...
let entry = bus.on("UserEvent", "handler", |event| async move {
    Ok(json!(event.inner.lock().payload.get("user_id").cloned()))
});
bus.on_sync("*", "wildcard_handler", |_event| Ok(Value::Null));
bus.on_with_options(
    "UserEvent",
    "custom",
    EventHandlerOptions {
...
```rust
let handler = bus.on_sync("UserEvent", "handler", |_event| Ok(serde_json::Value::Null));
bus.off("UserEvent", Some(&handler.id));
bus.off("*", None);
```
...
payload.insert("data".to_string(), json!("x"));
