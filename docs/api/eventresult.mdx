...
- event-lock acquisition in `EventBus::process_event(...)` via the global or bus serial lock
- `run_handler_with_context(...)` for dispatch context values
- `run_handler(...)`
- handler timeout resolution with event timeout capping
- slow-handler checks after callable completion
