...
// your Rust handler until retry parity is implemented.
```
...
```rust
// retry decorators/method wrappers are not implemented in abxbus-rust yet.
//
// Put retry/backoff code in a helper function or inside the handler body,
// then register that function as a normal EventBus handler.
```
...
//
// Put retry/backoff code in a helper function or inside the method body,
// then register that method/function as a normal EventBus handler.
```
...
- Rust and Go do not include `retry` yet.
