...
| Rust | `emit_base()` with `max_history_size > 0`, `max_history_drop=false`, and history already at limit | panic with `history limit reached` |
...
```rust
let event = bus.emit_base(BaseEvent::new("MyEvent", Default::default()));
