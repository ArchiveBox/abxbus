...
func main() {
	...
	handler := func(starts *[]int) func(context.Context, *abxbus.BaseEvent) (any, error) {
		return func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
			mu.Lock()
			inFlightGlobal++
...
bus_a.on_sync("WorkEvent", "handler_a", |_event| Ok(serde_json::Value::Null));
bus_b.on_sync("WorkEvent", "handler_b", |_event| Ok(serde_json::Value::Null));

for _ in 0..4 {
    bus_a.emit_base(BaseEvent::new("WorkEvent", Default::default()));
    bus_b.emit_base(BaseEvent::new("WorkEvent", Default::default()));
}
