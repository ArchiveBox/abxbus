...
bus_a.on::<SerialEvent, _, _>("handler", |_event| async move { Ok(()) });
bus_b.on::<SerialEvent, _, _>("handler", |_event| async move { Ok(()) });

for _ in 0..3 {
    bus_a.emit::<SerialEvent>(BaseEvent::new(SerialEvent {}));
    bus_b.emit::<SerialEvent>(BaseEvent::new(SerialEvent {}));
}
