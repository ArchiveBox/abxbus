...
func main() {
	...
	handler := func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		mu.Lock()
		inFlight++
...
bus_a.on_sync("SerialEvent", "handler", |_event| Ok(serde_json::Value::Null));
bus_b.on_sync("SerialEvent", "handler", |_event| Ok(serde_json::Value::Null));

for _ in 0..3 {
    bus_a.emit_base(BaseEvent::new("SerialEvent", Default::default()));
    bus_b.emit_base(BaseEvent::new("SerialEvent", Default::default()));
}
