...
func main() {
	...
	busA.OnEventName("SerialEvent", "handler", handler, nil)
	busB.OnEventName("SerialEvent", "handler", handler, nil)

	for i := 0; i < 3; i++ {
		busA.EmitEventName("SerialEvent", map[string]any{"order": i, "source": "a"})
		busB.EmitEventName("SerialEvent", map[string]any{"order": i, "source": "b"})
	}
