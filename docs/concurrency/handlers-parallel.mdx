...
func main() {
	...
	tracked := func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		mu.Lock()
		inFlight++
...
bus.on_sync("HandlerEvent", "tracked_a", |_event| Ok(serde_json::Value::Null));
bus.on_sync("HandlerEvent", "tracked_b", |_event| Ok(serde_json::Value::Null));
