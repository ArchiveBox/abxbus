...
bus.on::<HandlerEvent, _, _>("h1", |_event| async move { Ok(()) });
bus.on::<HandlerEvent, _, _>("h2", |_event| async move { Ok(()) });

let event = bus.emit::<HandlerEvent>(BaseEvent::new(HandlerEvent {}));
