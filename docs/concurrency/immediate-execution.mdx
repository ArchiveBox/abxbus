...
```rust
use abxbus_rust::{event_bus::EventBus, BaseEvent};
...
bus.on::<ParentEvent, _, _>("on_parent", |event| async move {
    let current_bus = event.event_bus().expect("handler bus");
    let child = current_bus.emit_child::<ChildEvent>(BaseEvent::new(EmptyPayload));
    child.wait_completed().await; // queue-jump while still inside this handler
    let value = child.event_result(Default::default()).await?;
    Ok(json!(format!("parent got: {value}")))
});

bus.on::<ChildEvent, _, _>("on_child", |_event| async move {
...
bus.On("ParentEvent", "on_parent", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	child := event.Emit(abxbus.NewBaseEvent("ChildEvent", nil))
	if _, err := child.Done(ctx); err != nil {
		return nil, err
	}
	value, err := child.EventResult(ctx)
	if err != nil {
		return nil, err
	}
...
bus.On("ChildEvent", "on_child", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
...
Python note: `asyncio.gather(..., return_exceptions=True)` is the closest `Promise.allSettled(...)` equivalent here. In Rust, call `wait_completed().await` on each emitted child to take the same immediate queue-jump path. In Go, call `Done(ctx)` on each emitted child.
...
bus.On("SiblingEvent", "on_sibling", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
...
- In Python, `await child_event.event_completed()` keeps normal queue order (non-queue-jump wait).
- In TypeScript, use `await child_event.done()`.
- In TypeScript, `await child_event.eventCompleted()` keeps normal queue order (non-queue-jump wait).
- In Go, use `childEvent.Done(ctx)` for the immediate path.
- In Go, use `childEvent.EventCompleted(ctx)` to keep normal queue order.
