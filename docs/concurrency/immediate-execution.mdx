...
bus.on(ParentEvent, |event: ParentEvent| async move {
    ...
    child.done().await; // queue-jump while still inside this handler
...
Python note: `asyncio.gather(..., return_exceptions=True)` is the closest `Promise.allSettled(...)` equivalent here. In Rust, call `done().await` on each emitted child to take the same immediate queue-jump path. In Go, call `Done(ctx)` on each emitted child.
