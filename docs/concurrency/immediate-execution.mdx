...
bus.on("ParentEvent", "on_parent", |event| async move {
    let current_bus = event.event_bus().expect("handler bus");
    let child = current_bus.emit_child_base(BaseEvent::new("ChildEvent", Map::new()));
...
bus.on_sync("ChildEvent", "on_child", |_event| {
