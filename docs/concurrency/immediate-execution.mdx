...
Python note: `asyncio.gather(..., return_exceptions=True)` is the closest `Promise.allSettled(...)` equivalent here. In Go, call `Done(ctx)` on each emitted child to take the same immediate queue-jump path.
