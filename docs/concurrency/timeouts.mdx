...
let event = BaseEvent::new("WorkEvent", Default::default());
{
    let mut inner = event.inner.lock();
    inner.event_timeout = Some(8.0);
    inner.event_handler_timeout = Some(2.0);
    inner.event_slow_timeout = Some(4.0);
    inner.event_handler_slow_timeout = Some(1.0);
}
...
```rust
use abxbus_rust::event_handler::EventHandlerOptions;

let entry = bus.on_sync_with_options(
    "WorkEvent",
    "slow_handler",
    EventHandlerOptions {
    ...
    |_event| Ok(serde_json::Value::Null),
);
```
...
```go
// EventBus.processEvent(...)
eventLock := bus.locks.getLockForEvent(event)
eventLock.Acquire(ctx)
defer eventLock.Release()

event.markStarted()
err := runWithTimeout(ctx, resolvedEventTimeout, eventTimeoutError, func(ctx context.Context) error {
	return event.runHandlers(ctx, bus, handlers, pendingResults, signalFirstHandlerStarted)
})

// BaseEvent.runHandlers(...) + EventBus.runHandler(...)
handlerLock := bus.locks.getLockForEventHandler(event, result)
handlerLock.Acquire(ctx)
defer handlerLock.Release()
...
```rust
// EventBus::process_event(...)
match resolved_event_concurrency {
    EventConcurrencyMode::GlobalSerial => {
        let _guard = EventBus::global_serial_lock().lock();
        self.process_event_inner(event).await;
    }
    EventConcurrencyMode::BusSerial => {
        let _guard = self.event_bus_serial_lock().lock();
        self.process_event_inner(event).await;
    }
    EventConcurrencyMode::Parallel => self.process_event_inner(event).await,
}

// EventBus::run_handler(...)
let effective_timeout = min_handler_and_event_timeout(...);
let timed_out = run_handler_with_context(event, handler, effective_timeout).await;
```
