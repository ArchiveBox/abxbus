...
```rust
use abxbus_rust::BaseEvent;

let event = BaseEvent::<WorkEvent>::new(WorkPayload::default());
{
...
let event = bus.emit::<WorkEvent>(event);
```
...
let entry = bus.on_with_options::<WorkEvent, _, _>(
    "slow_handler",
    EventHandlerOptions {
