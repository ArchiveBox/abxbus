...
bus.on::<WorkEvent, _, _>("bare_sync", |event| async move {
    Ok(format!("bare-sync:{}", event.event_payload().task_id))
});

bus.on::<WorkEvent, _, _>("bare_async", |event| async move {
    Delay::new(Duration::from_millis(10)).await;
    Ok(format!("bare-async:{}", event.event_payload().task_id))
});
...
bus.on::<WorkEvent, _, _>("instance_sync", move |event| async move {
    Ok(format!("{}-instance:{}", handlers.prefix, event.event_payload().task_id))
});
```
