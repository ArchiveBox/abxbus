...
impl EventSpec for WorkEvent {
    ...
    const EVENT_TYPE: &'static str = "WorkEvent";
}

let bus = EventBus::new(Some("AppBus".to_string()));

bus.on_typed_sync::<WorkEvent, _>("bare_sync", |event| {
    Ok(format!("bare-sync:{}", event.payload().task_id))
});

bus.on_typed::<WorkEvent, _, _>("bare_async", |event| async move {
    Delay::new(Duration::from_millis(10)).await;
    Ok(format!("bare-async:{}", event.payload().task_id))
});
...
bus.on_typed_sync::<WorkEvent, _>("instance_sync", move |event| {
    Ok(format!("{}-instance:{}", handlers.prefix, event.payload().task_id))
});
```
...
func bareHandler(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
...
func StaticStyleHandler(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	time.Sleep(10 * time.Millisecond)
	return fmt.Sprintf("static:%s", event.Payload["task_id"]), nil
}

func (h *HandlerSet) InstanceHandler(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
