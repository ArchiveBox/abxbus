...
Python/TypeScript AbxBus captures ambient context at `emit(...)` time and restores it when handlers execute. Go propagates the explicit `context.Context` passed to the event wait call. In both cases, handler code sees the same request-local values.
