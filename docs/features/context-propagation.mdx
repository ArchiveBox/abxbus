...
func main() {
	...
	bus.OnEventName("RequestEvent", "handler", func(event *abxbus.BaseEvent, ctx context.Context) (any, error) {
	...
	if _, err := bus.EmitEventNameWithContext(ctx, "RequestEvent", nil).Now(); err != nil {
		panic(err)
	}
}
```
