...
bus.on::<RequestEvent, _, _>("handler", |_event| async move {
...
let event = bus.emit::<RequestEvent>(BaseEvent::new(RequestEvent {}));
...
let event = bus.emit::<RequestEvent>(BaseEvent::new(EmptyPayload));
block_on(event.wait_completed());
