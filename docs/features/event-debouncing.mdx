...
```go
existing, err := bus.FindEventName("ScreenshotEvent", func(event *abxbus.BaseEvent) bool {
...
event := existing
if event == nil {
	event = bus.EmitEventName("ScreenshotEvent", map[string]any{"url": url})
}
...
```go
inFlight, err := bus.FindEventName("ScreenshotEvent", func(event *abxbus.BaseEvent) bool {
	return event.Payload["url"] == url
}, &abxbus.FindOptions{
	Past: false,  // skip history
...
event, err := bus.FindEventName("ScreenshotEvent", sameURL, &abxbus.FindOptions{Past: 10.0, Future: false})
if err != nil {
	panic(err)
}
if event == nil {
	event, err = bus.FindEventName("ScreenshotEvent", sameURL, &abxbus.FindOptions{Past: false, Future: 2.0})
	if err != nil {
		panic(err)
	}
}
...
func emitDebouncedScreenshot(ctx context.Context, bus *abxbus.EventBus, url string) (any, error) {
	...
	event, err := bus.FindEventName("ScreenshotEvent", sameURL, &abxbus.FindOptions{Past: 15.0, Future: false})
	if err != nil {
		return nil, err
	}
	if event == nil {
		event, err = bus.FindEventName("ScreenshotEvent", sameURL, &abxbus.FindOptions{Past: false, Future: 3.0})
		if err != nil {
			return nil, err
		}
	}
...
- Always await completion after selecting a debounced event:
  - Python: `await event.now()`, then `await event.event_result()`
  - TypeScript: `await event.now()`, then `await event.eventResult()`
  - Go: `event.Now()`, then `event.EventResult()`
