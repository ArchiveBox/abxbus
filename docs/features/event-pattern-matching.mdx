...
Go does not have runtime event classes in the same way Python and TypeScript do. It uses string event type names for registration/lookup, plus `OnTyped[...]` and typed constructors when you want static payload/result typing.
...
| String (`'UserActionEvent'`) | Events by type name | Dynamic routing/config-driven keys; primary Go pattern form |
