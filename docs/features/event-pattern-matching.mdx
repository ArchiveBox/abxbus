...
bus.on::<UserActionEvent, _, _>("on", |event| async move {
    Ok(format!("action:{}", event.event_payload().action))
});
...
- Rust: typed handlers use `bus.on::<MyEvent, _, _>(...)`; raw string/wildcard registration is reserved for low-level forwarding internals.
