...
Go and Rust use string event type names for low-level registration/lookup, plus typed helpers (`OnTyped[...]` in Go, `EventSpec`/`TypedEvent` in Rust) when you want static payload/result typing.
...
impl EventSpec for UserActionEvent {
    type Payload = UserActionPayload;
    type Result = String;
...
bus.on_typed_sync::<UserActionEvent, _>("on_typed", |event| {
    Ok(format!("action:{}", event.payload().action))
});

bus.on_sync("UserActionEvent", "on_by_name", |event| {
    println!("by-name {}", event.inner.lock().event_type);
    Ok(Value::Null)
});

bus.on_sync("*", "on_any", |event| {
    println!("wildcard {}", event.inner.lock().event_type);
    Ok(Value::Null)
});

let event = bus.emit::<UserActionEvent>(TypedEvent::new(UserActionPayload {
    action: "click".to_string(),
}));
block_on(event.wait_completed());

let typed_match = block_on(bus.find_typed::<UserActionEvent>(true, None));
...
func main() {
	...
	abxbus.OnTyped[UserActionPayload, string](
		...
		"UserActionEvent",
		"on_typed",
		func(ctx context.Context, payload UserActionPayload) (string, error) {
			return fmt.Sprintf("action:%s", payload.Action), nil
		},
		nil,
	)

	bus.On("UserActionEvent", "on_by_name", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		fmt.Println("by-name", event.EventType, event.Payload["action"])
		// by-name UserActionEvent click
		return nil, nil
	}, nil)

	bus.On("*", "on_any", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		fmt.Println("wildcard", event.EventType)
		// wildcard UserActionEvent
		return nil, nil
	}, nil)

	event, err := abxbus.NewTypedEventWithResult[UserActionPayload, string](
		"UserActionEvent",
	...
	_, err = bus.Emit(event).EventResult(context.Background())
	if err != nil {
		panic(err)
	}
...
- Rust: typed handlers use `on_typed(...)`; string/wildcard handlers receive `Arc<BaseEvent>`
- Go: typed handlers use `OnTyped[...]`; string/wildcard handlers receive `*BaseEvent`
