...
bus.on::<UserActionEvent, _, _>("on", |event| async move {
    Ok(format!("action:{}", event.payload().action))
});

let event = bus.emit::<UserActionEvent>(BaseEvent::new(UserActionEvent {
...
- Rust: typed handlers use `on::<EventSpec, _, _>(...)`; raw string/wildcard registration is reserved for low-level forwarding internals.
