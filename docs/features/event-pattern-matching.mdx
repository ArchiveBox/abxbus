...
func main() {
	...
	abxbus.OnTyped[UserActionPayload, string](
		...
		"UserActionEvent",
		"on",
		func(payload UserActionPayload, ctx context.Context) (string, error) {
			return fmt.Sprintf("action:%s", payload.Action), nil
		},
		nil,
	)

	bus.On("UserActionEvent", "on_by_name", func(event *abxbus.BaseEvent, ctx context.Context) (any, error) {
		fmt.Println("by-name", event.EventType, event.Payload["action"])
		// by-name UserActionEvent click
		return nil, nil
	}, nil)

	bus.On("*", "on_any", func(event *abxbus.BaseEvent, ctx context.Context) (any, error) {
