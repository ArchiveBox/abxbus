...
func main() {
	...
	bus.On("JobEvent", "on_job", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
...
struct JobEvent;
impl EventSpec for JobEvent {
    type Payload = JobPayload;
    type Result = ();
    const EVENT_TYPE: &'static str = "JobEvent";
}
...
bus.on::<JobEvent, _, _>("on_job", move |event| async move {
...
for order in 0..3 {
    bus.emit::<JobEvent>(BaseEvent::new(JobPayload { order }));
}
...
bus.On("SlowEvent", "on_slow", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
...
bus.On("FastEvent", "on_fast", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	trace = append(trace, fmt.Sprintf("start:%s:%s", event.EventType, event.Payload["name"]))
...
struct FastEvent;
impl EventSpec for FastEvent {
    type Payload = EmptyPayload;
    type Result = ();
    const EVENT_TYPE: &'static str = "FastEvent";
}
...
bus.on::<SlowEvent, _, _>("on_slow", move |_event| async move {
...
bus.on::<FastEvent, _, _>("on_fast", move |_event| async move {
    trace_for_fast.lock().unwrap().push("start:FastEvent".to_string());
    trace_for_fast.lock().unwrap().push("end:FastEvent".to_string());
    Ok(())
});

let slow = bus.emit::<SlowEvent>(BaseEvent::new(EmptyPayload));
let fast = bus.emit::<FastEvent>(BaseEvent::new(EmptyPayload));
block_on(bus.wait_until_idle(None));

assert!(slow.inner.inner.lock().event_completed_at <= fast.inner.inner.lock().event_started_at);
