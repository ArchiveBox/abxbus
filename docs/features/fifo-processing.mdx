...
func main() {
	...
	bus.OnEventName("JobEvent", "on_job", func(event *abxbus.BaseEvent) (any, error) {
	...
	emitted := []*abxbus.BaseEvent{
		bus.EmitEventName("JobEvent", map[string]any{"order": 0, "delay_ms": 30}),
		bus.EmitEventName("JobEvent", map[string]any{"order": 1, "delay_ms": 1}),
		bus.EmitEventName("JobEvent", map[string]any{"order": 2, "delay_ms": 20}),
	}
...
bus.OnEventName("SlowEvent", "on_slow", func(event *abxbus.BaseEvent) (any, error) {
...
bus.OnEventName("FastEvent", "on_fast", func(event *abxbus.BaseEvent) (any, error) {
	trace = append(trace, fmt.Sprintf("start:%s:%s", event.EventType, event.Payload["name"]))
...
slow := bus.EmitEventName("SlowEvent", map[string]any{"name": "slow-a"})
fast := bus.EmitEventName("FastEvent", map[string]any{"name": "fast-b"})
bus.WaitUntilIdle(nil)
