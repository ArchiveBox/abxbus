...
bus.on_sync("JobEvent", "on_job", move |event| {
    let order = event.inner.lock().payload["order"].as_i64().unwrap();
...
for order in 0..3 {
    ...
    bus.emit_base(BaseEvent::new("JobEvent", payload));
}
...
bus.on_sync("SlowEvent", "on_slow", move |_event| {
...
bus.on_sync("FastEvent", "on_fast", move |_event| {
    trace_for_fast.lock().unwrap().push("start:FastEvent".to_string());
    trace_for_fast.lock().unwrap().push("end:FastEvent".to_string());
    Ok(serde_json::Value::Null)
});

let slow = bus.emit_base(BaseEvent::new("SlowEvent", Default::default()));
let fast = bus.emit_base(BaseEvent::new("FastEvent", Default::default()));
block_on(bus.wait_until_idle(None));

assert!(slow.inner.lock().event_completed_at <= fast.inner.lock().event_started_at);
