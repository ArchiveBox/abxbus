...
func main() {
	...
	auth.bus.OnEventName("UserCreatedEvent", "on_user_created", func(event *abxbus.BaseEvent) (any, error) {
		return fmt.Sprintf("auth-ok:%s", event.Payload["user_id"]), nil
	}, nil)
	billing.bus.OnEventName("UserCreatedEvent", "on_user_created", func(event *abxbus.BaseEvent) (any, error) {
	...
	event := auth.bus.EmitEventName("UserCreatedEvent", map[string]any{"user_id": "u-a8d1"})
...
event := busA.EmitEventName("PingEvent", map[string]any{"message": "hello"})
