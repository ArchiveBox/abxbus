...
auth.on::<UserCreatedEvent, _, _>("on_user_created", |event| async move {
    Ok(json!(format!("auth-ok:{}", event.event_payload().user_id)))
});
billing.on::<UserCreatedEvent, _, _>("on_user_created", |event| async move {
    Ok(json!(format!("billing-ok:{}", event.event_payload().user_id)))
});

let relay_for_handler = relay.clone();
auth.on::<UserCreatedEvent, _, _>("forward_to_relay", move |event| {
...
relay.on::<UserCreatedEvent, _, _>("forward_to_billing", move |event| {
...
left.on::<PingEvent, _, _>("forward_to_right", move |event| {
...
right.on::<PingEvent, _, _>("forward_to_left", move |event| {
...
bus_a.on::<PingEvent, _, _>("forward_to_b", move |event| {
...
bus_b.on::<PingEvent, _, _>("forward_to_c", move |event| {
...
bus_c.on::<PingEvent, _, _>("forward_to_a", move |event| {
