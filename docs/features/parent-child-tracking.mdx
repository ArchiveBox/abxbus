...
Use `event.emit(...)` for owned child work. It keeps ancestry metadata intact and marks the child as blocking parent completion.
Use `bus.emit(...)` from a handler only for explicitly detached/background work that should keep ancestry but not hold the parent open unless later awaited.
