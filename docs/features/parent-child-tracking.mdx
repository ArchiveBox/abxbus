...
Most handler code should use `await event.emit(ChildEvent(...))` in Python or `await event.emit(ChildEvent(...)).done()` in TypeScript. That is the "owned child work" style: the child is linked to the parent, runs immediately from the caller's point of view, and the parent cannot complete until the child completes.
...
| --- | --- | --- | --- | --- |
| `await event.emit(ChildEvent(...))`<br />`await event.emit(ChildEvent(...)).done()` | Parent event id | `True` / `true` | Yes | Linked child work. The child can queue-jump, and parent completion waits for the child. |
...
| `await bus.emit(TopLevelEvent(...))`<br />`await bus.emit(TopLevelEvent(...)).done()` | `None` / `null` | `False` / `false` because there is no linked parent to block | Yes | Detached top-level event that the current handler waits for naturally because it is awaited. It is not a child in hierarchy tracking. |
...
- `current_bus.emit_child_base(BaseEvent::new(...))` creates linked child work while inside a handler.
- `child.wait_completed().await` queue-jumps and waits for that linked child.
- `bus.emit_base(BaseEvent::new(...))` creates detached top-level work.
- calling `wait_completed().await` on a bus-emitted event waits for it without adding parent-child lineage.

In Go, the same split is explicit:

- `child := event.Emit(abxbus.NewBaseEvent(...))` creates linked child work.
- `child.Done(ctx)` queue-jumps and waits for that linked child.
- `bus.Emit(abxbus.NewBaseEvent(...))` creates detached top-level work.
- calling `Done(ctx)` on a bus-emitted event waits for it without adding parent-child lineage.
...
bus.on("CheckoutEvent", "on_checkout", |event| async move {
    ...
    let order_id = event
        ...
        .get("order_id")
        .cloned()
        .unwrap_or_else(|| json!("ord-123"));

    let mut reserve_payload = Map::new();
    reserve_payload.insert("order_id".to_string(), order_id.clone());
    let reserve = current_bus.emit_child_base(BaseEvent::new("ReserveInventoryEvent", reserve_payload));
    reserve.wait_completed().await;
    let reserve_id = reserve.event_result(Default::default()).await?;

    let mut receipt_payload = Map::new();
    receipt_payload.insert("order_id".to_string(), order_id);
    let receipt = current_bus.emit_child_base(BaseEvent::new("SendReceiptEvent", receipt_payload));
    receipt.wait_completed().await;
    let receipt_id = receipt.event_result(Default::default()).await?;

    Ok(json!(format!("{reserve_id}|{receipt_id}")))
});

bus.on_sync("ReserveInventoryEvent", "on_reserve", |event| {
    Ok(json!(format!("reserve:{}", event.inner.lock().payload["order_id"])))
});
bus.on_sync("SendReceiptEvent", "on_receipt", |event| {
    Ok(json!(format!("receipt:{}", event.inner.lock().payload["order_id"])))
});

let mut payload = Map::new();
payload.insert("order_id".to_string(), json!("ord-123"));
let root = bus.emit_base(BaseEvent::new("CheckoutEvent", payload));
let result = block_on(root.event_result(Default::default()))?;
...
func main() {
	...
	bus.On("CheckoutEvent", "on_checkout", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		...
		if _, err := reserve.Done(ctx); err != nil {
			return nil, err
		}
		reserveID, err := reserve.EventResult(ctx)
		if err != nil {
			return nil, err
		}

		charge := event.Emit(abxbus.NewBaseEvent("ChargeCardEvent", map[string]any{"order_id": orderID}))
		if _, err := charge.Done(ctx); err != nil {
			return nil, err
		}
		chargeID, err := charge.EventResult(ctx)
		if err != nil {
			return nil, err
		}

		receipt := event.Emit(abxbus.NewBaseEvent("SendReceiptEvent", map[string]any{"order_id": orderID}))
		if _, err := receipt.Done(ctx); err != nil {
			return nil, err
		}
		receiptID, err := receipt.EventResult(ctx)
		if err != nil {
			return nil, err
		}
	...
	bus.On("ReserveInventoryEvent", "on_reserve", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		return fmt.Sprintf("reserve:%s", event.Payload["order_id"]), nil
	}, nil)

	bus.On("ChargeCardEvent", "on_charge", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		orderID := event.Payload["order_id"].(string)
		fraud := event.Emit(abxbus.NewBaseEvent("FraudCheckEvent", map[string]any{"order_id": orderID}))
		if _, err := fraud.Done(ctx); err != nil {
			return nil, err
		}
		fraudStatus, err := fraud.EventResult(ctx)
		if err != nil {
			return nil, err
		}
	...
	bus.On("FraudCheckEvent", "on_fraud", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
		return fmt.Sprintf("fraud-ok:%s", event.Payload["order_id"]), nil
	}, nil)
	bus.On("SendReceiptEvent", "on_receipt", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	...
	result, err := root.EventResult(ctx)
	if err != nil {
		panic(err)
	}
