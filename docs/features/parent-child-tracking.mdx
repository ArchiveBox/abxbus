...
- `child.done().await` queue-jumps and waits for that linked child.
- `bus.emit(EventType { ... })` creates detached top-level work.
- calling `done().await` on a bus-emitted event waits for it without adding parent-child lineage.
