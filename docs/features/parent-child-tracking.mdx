...
- `bus.emit::<EventType>(BaseEvent::new(...))` creates detached top-level work.
...
bus.on::<CheckoutEvent, _, _>("on_checkout", |event| async move {
    let current_bus = event.event_bus().expect("handler bus");
    let order_id = event.payload().order_id;

    let reserve = current_bus.emit_child::<ReserveInventoryEvent>(BaseEvent::new(ReserveInventoryEvent {
        order_id: order_id.clone(),
    }));
    reserve.wait_completed().await;
    let reserve_id = reserve.event_result(Default::default()).await?;

    let receipt = current_bus.emit_child::<SendReceiptEvent>(BaseEvent::new(SendReceiptEvent {
        order_id,
    }));
...
bus.on::<ReserveInventoryEvent, _, _>("on_reserve", |event| async move {
    Ok(json!(format!("reserve:{}", event.payload().order_id)))
});
bus.on::<SendReceiptEvent, _, _>("on_receipt", |event| async move {
    Ok(json!(format!("receipt:{}", event.payload().order_id)))
});

let root = bus.emit::<CheckoutEvent>(BaseEvent::new(OrderPayload {
