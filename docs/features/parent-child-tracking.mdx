...
- `current_bus.emit_child(ChildEvent { ... })` creates linked child work while inside a handler.
...
bus.on::<CheckoutEvent, _, _>("on_checkout", |event| async move {
    ...
    let order_id = event.event_payload().order_id;
...
bus.on::<ReserveInventoryEvent, _, _>("on_reserve", |event| async move {
    Ok(json!(format!("reserve:{}", event.event_payload().order_id)))
});
bus.on::<SendReceiptEvent, _, _>("on_receipt", |event| async move {
    Ok(json!(format!("receipt:{}", event.event_payload().order_id)))
});
