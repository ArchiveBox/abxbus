...
- Python: `event.emit(...)`
- TypeScript: `event.emit(...)`

Use `event.emit(...)` when child work should keep ancestry metadata. Awaiting that emitted child from the same handler marks it as blocking parent completion.
Leave `event.emit(...)` un-awaited for linked background work that should not hold the parent open. Use `bus.emit(...)` from a handler for detached root/background work with no parent link.
