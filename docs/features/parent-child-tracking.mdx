...
Most handler code should use `await event.emit(ChildEvent(...)).now()` in Python or TypeScript. That is the "owned child work" style: the child is linked to the parent, runs immediately from the caller's point of view, and the parent cannot complete until the child completes.
