...
func main() {
	...
	typedResult, err := abxbus.EventResultAs[int](result)
	if err != nil {
		panic(err)
	}
	_ = typedResult
...
bus.On("GetConfigEvent", "user_config", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	return map[string]any{"debug": true, "port": 8080}, nil
}, nil)
bus.On("GetConfigEvent", "system_config", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
	return map[string]any{"debug": false, "timeout": 30}, nil
}, nil)
...
//   map[string]any{"debug": false, "timeout": 30},
// }
...
// map[string]any{"debug": false, "port": 8080, "timeout": 30}
```
...
EventResultsList(
	ctx context.Context,
	include func(result any, eventResult *EventResult) bool,
	options *EventResultsListOptions,
) ([]any, error)
...
event.EventResultsList(ctx, func(result any, eventResult *abxbus.EventResult) bool {
	_, ok := result.(map[string]any)
	return ok
}, &abxbus.EventResultsListOptions{RaiseIfAny: false})
timeout := 0.25
