...
struct DoMathPayload {
    a: i64,
    b: i64,
}

struct DoMathEvent;
impl EventSpec for DoMathEvent {
    type Payload = DoMathPayload;
    type Result = i64;

    const EVENT_TYPE: &'static str = "DoMathEvent";
}

let bus = EventBus::new(Some("AppBus".to_string()));
bus.on_typed_sync::<DoMathEvent, _>("add", |event| {
    let payload = event.payload();
    Ok(payload.a + payload.b)
});

let event = bus.emit::<DoMathEvent>(TypedEvent::new(DoMathPayload { a: 2, b: 3 }));
...
event, err := abxbus.NewTypedEventWithResult[DoMathPayload, int](
	"DoMathEvent",
...
bus.on_sync("GetConfigEvent", "user_config", |_event| {
    Ok(json!({"debug": true, "port": 8080}))
});
bus.on_sync("GetConfigEvent", "system_config", |_event| {
    Ok(json!({"debug": false, "timeout": 30}))
});

let event = bus.emit_base(BaseEvent::new("GetConfigEvent", Map::new()));
