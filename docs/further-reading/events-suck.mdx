...
// For typed event APIs, use bus.On(...) and NewEvent(..., ResultType[T]()).
```
...
bus.OnEventName("CheckoutEvent", "on_checkout", func(event *abxbus.BaseEvent) (any, error) {
	...
	child := event.EmitEventName("ChargeCardEvent", map[string]any{"order_id": orderID})
...
Keep event payload fields and handler result types in the same event definition, then let handler signatures use the generated Rust struct directly.
