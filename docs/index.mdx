...
AbxBus is an in-memory event bus for async Python, TypeScript (Node and browser environments), Rust (alpha), and Go (alpha), built for predictable event-driven workflows with strong typing and consistent cross-language behavior.
...
struct SomeEvent;
impl EventSpec for SomeEvent {
    type Payload = SomePayload;
    type Result = String;
    const EVENT_TYPE: &'static str = "SomeEvent";
}

let bus = EventBus::new(Some("MyBus".to_string()));
bus.on("SomeEvent", "on_some_event", |event| async move {
    let data = event.inner.lock().payload["some_data"].clone();
    Ok(json!(format!("{data}")))
});

let event = bus.emit::<SomeEvent>(TypedEvent::new(SomePayload { some_data: 132 }));
...
func main() {
	...
	bus.On("SomeEvent", "on_some_event", func(ctx context.Context, event *abxbus.BaseEvent) (any, error) {
