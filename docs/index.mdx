...
bus.on::<SomeEvent, _, _>("on_some_event", |event| async move {
    Ok(format!("did something with {}", event.payload().some_data))
});

let event = bus.emit::<SomeEvent>(BaseEvent::new(SomeEvent { some_data: 132 }));
