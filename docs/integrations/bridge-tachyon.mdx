...
`TachyonEventBridge` forwards events between processes on the same machine via a single-producer/single-consumer shared-memory ring buffer (Unix-socket handshake, futex-backed wait). Each instance plays exactly one role per session — sender XOR listener — committed on the first call to `emit()` or `on()`.

## Optional dependencies

The native Tachyon core requires GCC 14+ or Clang 17+ at install time. It is not pulled in by default; install it explicitly when you want this bridge.
...
- Tachyon is SPSC: one bridge instance is either a producer or a consumer for the lifetime of its session. Use a separate bridge instance per direction if you need bidirectional forwarding.
...
- `close()` flushes a reserved `SHUTDOWN_TYPE_ID` sentinel from the producer so the consumer can exit its blocking `recv` loop without orphaned threads. Only the listener side unlinks the socket on disk.
