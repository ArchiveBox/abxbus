...
- In TypeScript, `close()` flushes a reserved `SHUTDOWN_TYPE_ID` sentinel from the producer so the consumer can exit its blocking `recv` loop. In Python, `close()` releases sender resources only; a listener blocked in `recv` is left as a daemon thread until process exit. Only the listener side unlinks the socket on disk.
